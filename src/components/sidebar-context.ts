 ...
 *
 * `AppShell` renders the **same** sidebar node twice — once as the sticky
 * desktop column, once inside the mobile drawer — so a Sidebar cannot tell the
 * two apart from its props. This context is how `AppShell` tells it: the
 * drawer copy is wrapped in a provider with `"drawer"`.
 *
